...
from .controller import HeightPID, AttitudePID, AttitudeMPC, HeightMPC
import numpy as np
...
class RollControlEnv:
    def __init__(self,
        ...
        self.height_controller = HeightMPC()
    ...
    def reset(self, target=None):
        ...
        # 初始化时便最好用float
        base_pos = target # np.array([0., 0., 0.])
        ...
        self.target = np.array([0., 0., 0.]) # (np.random.rand(3)-0.5)*2*5 if target is None else target
    ...
    def step(self, a):
        ...
        z_a = self.height_controller.solve(x_init=[self.current_pos[2], self.current_vel[2]],
                                           x_ref=[self.target[2], 0])
        ...
        self.uav.apply_action(f, tau_pitch, self.time)
        p.stepSimulation()
